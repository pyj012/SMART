...
class SocketServer():
    def __init__(self, port):
        ...
        # 필터 : 일단 개별 각도의 변동이 작은경우, 큰 경우 무시하는걸로 만들었는데 끝점 좌표가 많이 변하면 제한하는 방식이 나을지도?
            ...
            cv_thread = threading.Thread(target=self.run_mp, args=(input_stream1, input_stream2, P0, P1))
            cv_thread.daemon = True
            cv_thread.start()

            receive_thread = threading.Thread(target=self._handle_client)
        ...
        while True:
            try:
                ...
                for index in data_dict:
                    ...
                    self.decoded_img[index]= cv2.imdecode(data,1)

                for index in self.decoded_img:

                    if self.decoded_img[index] is not None:
                        cv2.imshow(str(index), self.decoded_img[index])
